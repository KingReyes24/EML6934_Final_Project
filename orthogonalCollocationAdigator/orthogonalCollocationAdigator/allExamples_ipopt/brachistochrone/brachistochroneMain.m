...
%-----------------------------------------------------------------%
global gravity psStuff nstates ncontrols
global iGfun jGvar
%-----------------------------------------------------------------%
...
xsize  = size(z0);
x      = adigatorCreateDerivInput(xsize,'z0');
output = adigatorGenJacFile('brachistochroneObj',{x});
grd_structure = output.JacobianStructure;

%-----------------------------------------------------------------%
% Set IPOPT callback functions
%-----------------------------------------------------------------%
funcs.objective   = @(Z)brachistochroneObj(Z);
funcs.gradient    = @(Z)brachistochroneGrd(Z);
funcs.constraints = @(Z)brachistochroneCon(Z);
funcs.jacobian    = @(Z)brachistochroneJac(Z);
funcs.jacobianstructure = @()brachistochroneJacPat(S_jac);
